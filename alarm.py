...
def get_alarm_mysql_status():
    ...
    if result <> 0:
        for line in result:
            ...
            tags=line[21]
            db_type=line[22]
	    func.check_db_status(server_id,host,port,tags,db_type)
            ...
            if connect <> 1:
                ...
                updStatuss = []
                updStatuss.append(['connect','3',create_time,'connect','down','critical'])
                updStatuss.append(['sessions','-1'])
                updStatuss.append(['actives','-1'])
                updStatuss.append(['waits','-1'])
                updStatuss.append(['repl','-1'])
                updStatuss.append(['repl_delay','-1'])
                func.update_db_status_more(updStatuss, host, port)
            else:
...
def get_alarm_oracle_status():
    sql = "select a.server_id,a.connect, a.database_role,a.session_total,a.session_actives,a.session_waits,a.create_time,b.host,b.port,b.alarm_session_total,b.alarm_session_actives,b.alarm_session_waits,b.threshold_warning_session_total,b.threshold_critical_session_total,b.threshold_warning_session_actives,b.threshold_critical_session_actives,b.threshold_warning_session_waits,b.threshold_critical_session_waits,a.dg_delay, a.dg_stats, b.alarm_repl_status, b.alarm_repl_delay, b.threshold_warning_repl_delay, b.threshold_critical_repl_delay,b.send_mail,b.send_mail_to_list,b.send_sms,b.send_sms_to_list,b.tags,'oracle' as db_type from oracle_status a, db_servers_oracle b where a.server_id=b.id;"
    result=func.mysql_query(sql)
    if result <> 0:
        for line in result:
	    (server_id, connect, database_role, session_total, session_actives, session_waits, create_time, host, port, alarm_session_total, alarm_session_actives, alarm_session_waits, threshold_warning_session_total, threshold_critical_session_total, threshold_warning_session_actives, threshold_critical_session_actives, threshold_warning_session_waits, threshold_critical_session_waits, dg_delay, dg_stats, alarm_repl_status, alarm_repl_delay, threshold_warning_repl_delay, threshold_critical_repl_delay, send_mail, send_mail_to_list, send_sms, send_sms_to_list, tags,db_type) = line
            func.check_db_status(server_id,host,port,tags,db_type)
            if send_mail_to_list is None or  send_mail_to_list.strip()=='':
                send_mail_to_list = mail_to_list_common
            if send_sms_to_list is None or  send_sms_to_list.strip()=='':
		...
		'''
            ...
            if cmp("PRIMARY", database_role) and int(alarm_repl_status)==1:
                if not cmp("VALID", dg_stats):
                    func.check_if_ok(server_id,tags,host,port,create_time,db_type,'replication','dg_stats:'+dg_stats,'replication ok',send_mail,send_mail_to_list,send_sms,send_sms_to_list)
                    func.update_db_status('repl',1,host,port,create_time,'replication','dg_stats:'+dg_stats,'ok')
                    if int(alarm_repl_delay)==1:
                        if int(dg_delay)>=int(threshold_critical_repl_delay):
                            send_mail = func.update_send_mail_status(server_id,db_type,'repl_delay',send_mail,send_mail_max_count) 
                            send_sms = func.update_send_sms_status(server_id,db_type,'repl_delay',send_sms,send_sms_max_count) 
                            func.add_alarm(server_id,tags,host,port,create_time,db_type,'repl_delay',dg_delay,'critical','dg has delay',send_mail,send_mail_to_list,send_sms,send_sms_to_list)
                            func.update_db_status('repl_delay',3,host,port,create_time,'repl_delay',dg_delay,'critical')
                        elif int(dg_delay)>=int(threshold_warning_repl_delay):
                            send_mail = func.update_send_mail_status(server_id,db_type,'repl_delay',send_mail,send_mail_max_count) 
                            send_sms = func.update_send_sms_status(server_id,db_type,'repl_delay',send_sms,send_sms_max_count) 
                            func.add_alarm(server_id,tags,host,port,create_time,db_type,'repl_delay',dg_delay,'warning','replication has delay',send_mail,send_mail_to_list,send_sms,send_sms_to_list)
                            func.update_db_status('repl_delay',2,host,port,create_time,'repl_delay',dg_delay,'warning')
                        else:
		    ...
		    send_mail = func.update_send_mail_status(server_id,db_type,'replication',send_mail,send_mail_max_count)
		    send_sms = func.update_send_sms_status(server_id,db_type,'replication',send_sms,send_sms_max_count) 
		    func.add_alarm(server_id,tags,host,port,create_time,db_type,'replication','dg:'+dg_stats,'critical','replication stop',send_mail,send_mail_to_list,send_sms,send_sms_to_list)
		    func.update_db_status('repl',3,host,port,create_time,'replication','dg:'+dg_stats,'critical')
		    func.update_db_status('repl_delay','-1',host,port,'','','','')	
	    else:
		func.update_db_status('repl','-1',host,port,'','','','')	
		func.update_db_status('repl_delay','-1',host,port,'','','','')	
...
def get_alarm_oracle_tablespace():
    ...
    if result <> 0:
        for line in result:
            ...
            used_rate_int=int(used_rate.rstrip("%"))
...
def get_alarm_os_status():
    ...
    if result <> 0:
	    ...
	    host=line[0]
            ...
            if zabbix <> 1:
                send_mail = func.update_send_mail_status(host,db_type,'zabbix_server',send_mail,send_mail_max_count)
                send_sms = func.update_send_sms_status(host,db_type,'zabbix_server',send_sms,send_sms_max_count)
                func.add_alarm(server_id,tags,host,port,create_time,db_type,'zabbix_server','down','critical','zabbix server down',send_mail,send_mail_to_list,send_sms,send_sms_to_list)
		'''
                func.update_db_status('zabbix','3',host,'',create_time,'zabbix_server','down','critical')
		...
		'''
		updStatuss = []
                updStatuss.append(['zabbix','3',create_time,'zabbix_server','down','critical'])
                updStatuss.append(['process','-1'])
                updStatuss.append(['load_1','-1'])
                updStatuss.append(['cpu','-1'])
                updStatuss.append(['memory','-1'])
                updStatuss.append(['network','-1'])
                updStatuss.append(['disk','-1',])
                func.update_db_status_more(updStatuss, host)
            else:
                func.check_if_ok(server_id,tags,host,port,create_time,db_type,'zabbix_server','up','zabbix server up',send_mail,send_mail_to_list,send_sms,send_sms_to_list)
                func.update_db_status('zabbix',1,host,'',create_time,'zabbix_server','up','ok')
                ...
                if int(alarm_os_memory)==1:
                    if memory_usage:
                        memory_usage_int = int(float(memory_usage.rstrip('%')))
                    else:
                        memory_usage_int = 0 
...
def check_send_alarm_sleep():
    ...
    if send_mail_sleep_time:
        ...
        sql="delete from alarm_temp where alarm_type='mail' and create_time <= date_sub(now(), interval %s second)" % (send_mail_sleep_time)
        func.mysql_exec(sql)
    ...
    if send_sms_sleep_time:
        ...
        sql="delete from alarm_temp where alarm_type='mail' and create_time <= date_sub(now(), interval %s second)" % (send_sms_sleep_time)
        func.mysql_exec(sql)


def get_alarm_mysql():
    print "alarm monitor mysql" 
    get_alarm_mysql_status()
    get_alarm_mysql_replcation()
    func.update_check_time() 

def get_alarm_oracle():
    print "alarm monitor oracle" 
    get_alarm_oracle_status()
    get_alarm_oracle_tablespace()
    func.update_check_time() 

def get_alarm_mongodb():
    print "alarm monitor mongodb" 
    get_alarm_mongodb_status()
    func.update_check_time() 

def get_alarm_redis():
    print "alarm monitor redis" 
    get_alarm_redis_status()
    func.update_check_time() 

def get_alarm_os():
    print "alarm monitor OS" 
    get_alarm_os_status()
    get_alarm_os_disk()
...
def main():
    ...
    joblist = [] 
    if monitor_mysql=="1":
	job = Process(target = get_alarm_mysql, args = ())
	joblist.append(job)
	job.start()
        
    if monitor_oracle=="1":
	time.sleep(3)
	job = Process(target = get_alarm_oracle, args = ())
	joblist.append(job)
	job.start()
        
    if monitor_mongodb=="1":
	time.sleep(3)
	job = Process(target = get_alarm_mongodb, args = ())
	joblist.append(job)
	job.start()
        
    if monitor_redis=="1":
	time.sleep(3)
	job = Process(target = get_alarm_redis, args = ())
	joblist.append(job)
	job.start()

    if monitor_os=="1":
	time.sleep(3)
	job = Process(target = get_alarm_os, args = ())
	joblist.append(job)
	job.start()
        
    exeTimeout = 100
    for job in joblist:
	job.join(timeout=exeTimeout)
    print "send_alarm" 
    send_alarm()
    func.update_check_time() 

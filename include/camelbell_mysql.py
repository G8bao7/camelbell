...
def get_sessions(conn):
    try:
        curs=conn.cursor()
        curs.execute("select count(*) from processlist");
...
def get_actives(conn):
    try:
        ...
        curs.execute("select count(1) from processlist where command not in ('Sleep') ");
        result = curs.fetchone()[0]
        return result
...
def get_waits(conn):
    try:
        ...
        curs.execute("select count(1) from processlist where state like '%wait%' and state != 'Waiting for an event from Coordinator'");
        result = curs.fetchone()[0]
        return result
